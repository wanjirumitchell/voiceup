...
def bulk_email():
    ...
    if request.method == 'POST':
        ...
        if audience == 'students':
            cursor.execute("SELECT email, fullname FROM users WHERE is_banned=0 AND role='student'")
            ...
            cursor.execute("SELECT email, fullname FROM users WHERE is_banned=0")
    ...
    total_users = cursor.fetchone()['cnt']
    cursor.execute("SELECT COUNT(*) as cnt FROM users WHERE is_banned=0 AND role='student'")
    total_students = cursor.fetchone()['cnt']
    cursor.execute("""
        SELECT COUNT(DISTINCT u.id) as cnt FROM users u
        JOIN suggestions s ON u.id=s.user_id WHERE u.is_banned=0
    """)
