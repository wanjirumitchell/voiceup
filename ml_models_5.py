...
# ═══════════════════════════════════════════════════════════════════
# 7. STUDENT ENGAGEMENT SCORER
# Scores students on QUALITY not just quantity:
#   - Resolution rate (did suggestions get resolved?)
#   - Vote impact (did others support the ideas?)
#   - Description quality (length/detail)
#   - Consistency (spread of submissions over time)
#   - Sentiment quality (constructive vs angry)
# Returns a 0-100 engagement score per student.
# ═══════════════════════════════════════════════════════════════════

def compute_engagement_score(student_data):
    """
    Compute engagement score for a student.

    student_data: dict with keys:
        total_suggestions, resolved_count, rejected_count,
        total_votes, avg_description_length, weeks_active,
        avg_sentiment_score (0-1, optional)

    Returns: (score: int 0-100, breakdown: dict)
    """
    total = student_data.get('total_suggestions', 0)
    if total == 0:
        return 0, {}

    resolved   = student_data.get('resolved_count', 0)
    rejected   = student_data.get('rejected_count', 0)
    votes      = student_data.get('total_votes', 0)
    avg_desc   = student_data.get('avg_description_length', 50)
    weeks      = max(student_data.get('weeks_active', 1), 1)
    sentiment  = student_data.get('avg_sentiment_score', 0.5)

    # ── Component 1: Resolution rate (30 points) ──
    # Suggestions that actually got resolved = high quality
    resolution_rate = resolved / total
    resolution_score = round(resolution_rate * 30)

    # ── Component 2: Vote impact (25 points) ──
    # Avg votes per suggestion (capped at 10 avg = full score)
    avg_votes = votes / total
    vote_score = round(min(avg_votes / 10, 1) * 25)

    # ── Component 3: Description quality (20 points) ──
    # Well-written suggestions (100+ chars = detailed)
    if avg_desc >= 200:
        quality_score = 20
    elif avg_desc >= 100:
        quality_score = 14
    ...
    # Spread over time = engaged, not spamming
    submissions_per_week = total / weeks
    if 0.5 <= submissions_per_week <= 3:
        consistency_score = 15   # Sweet spot — regular contributor
    elif submissions_per_week < 0.5:
        consistency_score = 10   # Occasional
    else:
        consistency_score = 6    # Spamming

    # ── Component 5: Constructiveness (10 points) ──
    # Positive/constructive sentiment = quality contributor
    sentiment_score = round(sentiment * 10)

    total_score = min(
        resolution_score + vote_score + quality_score +
        consistency_score + sentiment_score, 100
    )

    breakdown = {
        'resolution':    resolution_score,
        'vote_impact':   vote_score,
        'quality':       quality_score,
        'consistency':   consistency_score,
        'constructive':  sentiment_score,
        'total':         total_score,
    }
...
def get_engagement_tier(score):
    """Return tier label and icon for an engagement score."""
    if score >= 80:
        return 'Champion', '🏆', '#f59e0b'
    elif score >= 60:
        return 'Active Voice', '🌟', '#6366f1'
    elif score >= 40:
        return 'Contributor', '✅', '#10b981'
    elif score >= 20:
        return 'Starter', '🌱', '#3b82f6'
    else:
        return 'New Member', '👋', '#94a3b8'


# ═══════════════════════════════════════════════════════════════════
# 8. ANOMALY DETECTION
# Detects unusual spikes in suggestion volume per category.
# Uses a simple statistical approach: Z-score on daily counts.
# A Z-score > 2.0 = significantly above normal = anomaly.
# No ML training needed — purely statistical.
# ═══════════════════════════════════════════════════════════════════

def detect_anomalies(daily_counts_by_category, window_days=14, z_threshold=2.0):
    """
    Detect anomalous spikes in suggestion volume.

    daily_counts_by_category: dict of {category: [(date, count), ...]}
        Each list is sorted by date, most recent last.

    Returns list of anomaly dicts sorted by severity:
        {category, date, count, expected, z_score, severity, message}
    """
    anomalies = []

    for category, daily_data in daily_counts_by_category.items():
        if len(daily_data) < 3:
            continue   # Need at least 3 data points

        counts = [d[1] for d in daily_data]

        # Baseline: mean and std of the window (excluding today)
        ...
        if z_score >= z_threshold:
            ...
            # Friendly message
            pct_above = round(((today_count - mean) / max(mean, 1)) * 100)
            message = (
                f"{today_count} {category} suggestion{'s' if today_count != 1 else ''} "
                f"today — {pct_above}% above the {window_days}-day average "
                f"({round(mean, 1)}/day). This may indicate a significant issue."
            )
