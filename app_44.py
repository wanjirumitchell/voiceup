...
def ai_sentiment_dashboard():
    ...
    cursor.execute("""
        SELECT DATE_FORMAT(aa.analyzed_at, '%Y-%m') as month,
               aa.sentiment, COUNT(*) as cnt
        FROM ai_analysis aa
        WHERE aa.analyzed_at >= DATE_SUB(NOW(), INTERVAL 6 MONTH)
        GROUP BY month, aa.sentiment
        ORDER BY month
    """)
    ...
    cursor.execute("""
        SELECT s.title, s.suggestion_id, s.category, s.vote_count,
